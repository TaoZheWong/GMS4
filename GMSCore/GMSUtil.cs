...
namespace GMSCore
{
	public sealed class GMSUtil
	{
		...
		public static readonly string LOG_FILE = string.IsNullOrEmpty(ConfigurationManager.AppSettings["LOG_FILE"]) ? AppDomain.CurrentDomain.BaseDirectory + "Log.txt" : ConfigurationManager.AppSettings["LOG_FILE"];
		...
		#region WriteLog
		/// <summary>
		/// Static function to write to a log file
		/// </summary>
		/// <param name="message">String value of the Message</param>
		/// <param name="fileName">String value of the file path and name</param>
		public static void WriteLog(string message)
		{
			System.IO.StreamWriter writer = null;
			try
			{
				if (System.IO.File.Exists(LOG_FILE) == true)
					writer = System.IO.File.AppendText(LOG_FILE);
				else
					writer = System.IO.File.CreateText(LOG_FILE);

				writer.WriteLine("-----------------------------------------------------------------------");
				writer.Write("[");
				writer.Write(DateTime.Now.ToString("dd-MMM-yy HH:mm:ss"));
				writer.WriteLine("] :");
...
#region document number control 
#endregion
