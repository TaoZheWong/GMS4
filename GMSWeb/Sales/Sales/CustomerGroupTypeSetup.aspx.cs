...
{
    public partial class CustomerGroupTypeSetup : GMSBasePage
    {
        ...
        private void RetrieveCustomerWithGroupType()
        {
            LogSession session = base.GetSessionInfo();
            ...
            try
            {
                ggdal.RetrieveCustomerWithGroupType(session.CompanyId, session.UserId, ref ds);
            }
            catch (Exception ex)
            {
        ...
        protected void RadGrid1_OnUpdateCommand(object source, GridCommandEventArgs e)
        {
            ...
            if (e.Item is GridEditableItem && e.Item.IsInEditMode && !(e.Item is IGridInsertItem))
            {
                try
                {
                    ...
                    string accountCode = editableItem.GetDataKeyValue("AccountCode").ToString();
                    DropDownList ddlType = editableItem.FindControl("ddlType") as DropDownList;
                    string type = ddlType.SelectedValue;
                    TextBox txtGroup = editableItem["Group"].Controls[0] as TextBox;
                    string group = "-";
                    //if (string.IsNullOrEmpty(txtGroup.Text))
                    //{
                    //    //alertMessage("Do not leave blank!");
                    //    //e.Canceled = true;
                    //}
                    if (!string.IsNullOrEmpty(txtGroup.Text))
                        group = txtGroup.Text;
                    AccountInformation ai = AccountInformation.RetrieveByKey(session.CompanyId, accountCode);
                    if (ai != null)
                    {
                        ai.AccountClass = type;
                        ai.AccountGroup = group;
                        ai.Save();
                        alertMessage("Customer Updated.");
                    }
                    else
                    {
                        alertMessage("Customer not exists");
                        e.Canceled = true;
                        return;
                    }
                }
        ...
        protected void RadGrid1_OnItemCreated(object sender, GridItemEventArgs e)
        {
            ...
            if (e.Item is GridEditableItem && e.Item.IsInEditMode)
            {
                ...
                IList<AccountClass> lstAccClass = null;
        ...
        protected void RadGrid1_ItemDataBound(object sender, GridItemEventArgs e)
        {
            ...
            if (e.Item is GridEditableItem && e.Item.IsInEditMode && !(e.Item is IGridInsertItem))
            {
                GridEditableItem editableItem = e.Item as GridEditableItem;
                DropDownList ddlType = editableItem.FindControl("ddlType") as DropDownList;
                HtmlInputHidden hidType = (HtmlInputHidden)editableItem.FindControl("hidType");
