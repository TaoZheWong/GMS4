...
{
    public partial class BrandProductSetup : GMSBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                RetrieveBrandProductByCategory();
        ...
        private void RetrieveBrandProductByCategory()
        {
            LogSession session = base.GetSessionInfo();
            ...
            try
            {
                ggdal.RetrieveBrandProductByCategory(session.CompanyId, loginUserOrAlternateParty, ref ds);
            }
            catch (Exception ex)
            {
        ...
        protected void RadGrid1_FilterCheckListItemsRequested(object sender, GridFilterCheckListItemsRequestedEventArgs e)
        {
            ...
            try
            {
                ggdal.RetrieveBrandProductByCategory(session.CompanyId, session.UserId, ref ds);
            }
            catch (Exception ex)
            {
                alertMessage(ex.ToString());
        ...
        protected void RadGrid1_OnUpdateCommand(object source, GridCommandEventArgs e)
        {
            ...
            if (e.Item is GridEditableItem && e.Item.IsInEditMode && !(e.Item is IGridInsertItem))
            {
                try
                {
                    ...
                    DropDownList ddlMainCategory = editableItem.FindControl("ddlMainCategory") as DropDownList;
                    string mainCategory = "";
                    if (!string.IsNullOrEmpty(ddlMainCategory.SelectedValue))
                        mainCategory = ddlMainCategory.SelectedValue;
        ...
        protected void RadGrid1_OnItemCreated(object sender, GridItemEventArgs e)
        {
            ...
            if (e.Item is GridEditableItem && e.Item.IsInEditMode)
            {
                ...
                DropDownList ddlMainCategory = editableItem.FindControl("ddlMainCategory") as DropDownList;
                ggdal.RetrieveBudgetBrand(session.CompanyId,session.UserId,ref dsBrand);
                for (int j = 0; j < dsBrand.Tables[0].Rows.Count; j++)
                {
                    ddlMainCategory.Items.Add(new ListItem(dsBrand.Tables[0].Rows[j]["BrandName"].ToString(), dsBrand.Tables[0].Rows[j]["BudgetCode"].ToString()));
                }
            }
        }
        ...
        protected void RadGrid1_ItemDataBound(object sender, GridItemEventArgs e)
        {
            ...
            if (e.Item is GridEditableItem && e.Item.IsInEditMode && !(e.Item is IGridInsertItem))
            {
                ...
                HtmlInputHidden hidMainCategory = (HtmlInputHidden)editableItem.FindControl("hidMainCategory");
                ...
                if (!string.IsNullOrEmpty(hidMainCategory.Value))
                {
                    ddlMainCategory.SelectedValue = hidMainCategory.Value;
                }
            }
        }
        #endregion
