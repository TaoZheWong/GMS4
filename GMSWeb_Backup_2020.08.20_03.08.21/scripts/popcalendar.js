...
//	email  : pinotoy@yahoo.com
	...
	var imgDir = '../../images/CalendarIcon/';		// directory for images ... e.g. var imgDir="/img/"
	var dayName = '';
	...
	*/
	...
	if (dom) {
		...
		document.write ('<div onclick="bShow=true" id="calendar" style="z-index:+99999;position:absolute;visibility:hidden;"><table width="'+((showWeekNumber==1)?250:220)+'" style="font-family:Arial;font-size:2px;border: 1px solid #A0A0A0;" bgcolor="#ffffff"><tr bgcolor="#000066"><td><table width="'+((showWeekNumber==1)?248:218)+'"><tr><td style="padding:2px;font-family:Arial;font-size:2px;"><font color="#ffffff' + '' /*C9D3E9*/ +'" size="2px"><b><span id="caption"></span></b></font></td><td align="right"><a href="javascript:hideCalendar()"><img src="'+imgDir+'close.gif" width="15" height="13" border="0" /></a></td></tr></table></td></tr><tr><td style="padding:5px" bgcolor="#ffffff"><span id="content"></span></td></tr>');

		if (showToday == 1) {
			document.write ('<tr bgcolor="#f0f0f0"><td style="padding:5px" align="center"><span id="lblToday"></span></td></tr>');
		}
			
		document.write ('</table></div><div id="selectMonth" style="z-index:+999999;position:absolute;visibility:hidden;"></div><div id="selectYear" style="z-index:+999999;position:absolute;visibility:hidden;"></div>');
	}
	...
	function constructMonth() {
		...
		if (!monthConstructed) {
			...
			for (i=0; i<12; i++) {
				...
				sHTML += '<tr><td id="m' + i + '" onmouseover="this.style.backgroundColor=\'#909090\'" onmouseout="this.style.backgroundColor=\'\'" style="cursor:pointer" onclick="monthConstructed=false;monthSelected=' + i + ';constructCalendar();popDownMonth();event.cancelBubble=true"><font color="#000066" size="2px">&nbsp;' + sName + '&nbsp;</font></td></tr>';
			}

			document.getElementById('selectMonth').innerHTML = '<table width="70" style="font-family:Arial;font-size:2px;border:1px solid #a0a0a0;" bgcolor="#f0f0f0" cellspacing="0" onmouseover="clearTimeout(timeoutID1)" onmouseout="clearTimeout(timeoutID1);timeoutID1=setTimeout(\'popDownMonth()\',100);event.cancelBubble=true">' + sHTML + '</table>';
	...
	function incYear() {
		for	(i=0; i<7; i++) {
			...
			if (newYear == yearSelected)
				txtYear = '<span style="color:#006;font-weight:bold;font-size=2px;">&nbsp;' + newYear + '&nbsp;</span>';
			else
	...
	function constructYear() {
		...
		sHTML = '';
		if (!yearConstructed) {
			sHTML = '<tr><td align="center" onmouseover="this.style.backgroundColor=\'#909090\'" onmouseout="clearInterval(intervalID1);this.style.backgroundColor=\'\'" style="cursor:pointer" onmousedown="clearInterval(intervalID1);intervalID1=setInterval(\'decYear()\',30)" onmouseup="clearInterval(intervalID1)"><font color="#000066" size="2px">-</font></td></tr>';
			...
			for ( i = (yearSelected-3); i <= (yearSelected+3); i++ ) {
				...
				sHTML += '<tr><td id="y' + j + '" onmouseover="this.style.backgroundColor=\'#909090\'" onmouseout="this.style.backgroundColor=\'\'" style="cursor:pointer" onclick="selectYear('+j+');event.cancelBubble=true"><font color="#000066" size="2px">&nbsp;' + sName + '&nbsp;</font></td></tr>';
				j++;
			}

			sHTML += '<tr><td align="center" onmouseover="this.style.backgroundColor=\'#909090\'" onmouseout="clearInterval(intervalID2);this.style.backgroundColor=\'\'" style="cursor:pointer" onmousedown="clearInterval(intervalID2);intervalID2=setInterval(\'incYear()\',30)" onmouseup="clearInterval(intervalID2)"><font color="#000066" size="2px">+</font></td></tr>';

			document.getElementById('selectYear').innerHTML = '<table width="44" cellspacing="0" bgcolor="#f0f0f0" style="font-family:Arial;font-size:2px;border:1px solid #a0a0a0;" onmouseover="clearTimeout(timeoutID2)" onmouseout="clearTimeout(timeoutID2);timeoutID2=setTimeout(\'popDownYear()\',100)">' + sHTML + '</table>';
	...
	function constructCalendar () {
		...
		sHTML = '<table border="0" style="font-family:verdana;font-size:2px;"><tr>';
		...
		for (i = 0; i<7; i++) {
			sHTML += '<td width="27" align="right"><b><font color="#000066" size="2px">' + dayName[language][i] + '</font></b></td>';
		}
		...
		for	( datePointer=1; datePointer <= numDaysInMonth; datePointer++ ) {
			...
			if ((datePointer == dateNow) && (monthSelected == monthNow) && (yearSelected == yearNow)) {	///// today
				sHTML += "<b><a "+dateMessage+" title=\"" + sHint + "\" style='"+sStyle+"' "+selDayAction+"><font color=#ff0000 size=2px>&nbsp;" + datePointer + "</font>&nbsp;</a></b>";
			} else if (dayPointer % 7 == (startAt * -1)+1) {									///// SI ES DOMINGO
				if (isPast==1)
					sHTML += "<a "+dateMessage+" title=\"" + sHint + "\" style='"+sStyle+"' "+selDayAction+">&nbsp;<font color=#909090 size=2px>" + datePointer + "</font>&nbsp;</a>";
				else
					sHTML += "<a "+dateMessage+" title=\"" + sHint + "\" style='"+sStyle+"' "+selDayAction+">&nbsp;<font color=#54A6E2 size=2px>" + datePointer + "</font>&nbsp;</a>";
				...
				if (isPast==1)
					...
					sHTML += "<a "+dateMessage+" title=\"" + sHint + "\" style='"+sStyle+"' "+selDayAction+">&nbsp;<font color=#000066 size=2px>" + datePointer + "</font>&nbsp;</a>";
			}
	...
	//call this method.
	function showCalendar2(ctl, from, to, format, lang, past, fx, fy) {	   
	    
	    var edited = document.getElementById(ctl.id.replace(from, to)); 
	    ...
	    popUpCalendar(ctl, edited, format);
    ...
    function popUpCalendar(ctl, ctl2, format) {   
		...
		if (bPageLoaded) {
			...
			if (crossobj.visibility == 'hidden') {
				...
				//Modified by Siew Siew Ong on 4 Jul 2013
				//Fix date cannot be displayed in non-IE browser. 
				var left = 0; 
				var l1 = ctl.offsetLeft; 
				var l2 = (leftpos-220); 
				left = ctl.offsetLeft + (leftpos-220); 
				
				var top = 0; 
				var t1 = ctl.offsetTop; 
				var t2 = toppos; 
				var t3 = ctl.offsetHeight; 
				top = t1 + t2 + t3 + 2; 
				
				//crossobj.left = (fixedX == -1) ? ctl.offsetLeft + (leftpos-220) : fixedX;
				//crossobj.top = (fixedY == -1) ? ctl.offsetTop + toppos + ctl.offsetHeight + 2 : fixedY;
				crossobj.left = (fixedX == -1) ? left + "px" : fixedX + "px"; 
				crossobj.top = (fixedY == -1) ? top + "px" : fixedY + "px"; 
