ALTER PROCEDURE [dbo].[procReportFinanceSAPPNLInRAW]
@CoyID smallint, 
@ProjectID smallint,
...
@Division nvarchar(100), @Department nvarchar(100), @Section nvarchar(100), @Less float, @raw smallint,
...
CASE WHEN F.ItemSeqID > 54 THEN F.MTD * @raw ELSE F.MTD/@Less * @raw END AS MTD, 
CASE WHEN F.ItemSeqID > 54 THEN F.YTD * @raw ELSE F.YTD/@Less * @raw END AS YTD, 
CASE WHEN F.ItemSeqID > 54 THEN LM.MTD * @raw ELSE LM.MTD/@Less * @raw END AS LM_MTD, 
CASE WHEN F.ItemSeqID > 54 THEN MTDBudgetTotal * @raw ELSE MTDBudgetTotal/@Less * @raw END AS MTDBudgetTotal, 
CASE WHEN F.ItemSeqID > 54 THEN YTDBudgetTotal * @raw ELSE YTDBudgetTotal/@Less * @raw END AS YTDBudgetTotal, 
CASE WHEN F.ItemSeqID > 54 THEN L.MTD * @raw ELSE L.MTD/@Less * @raw END AS LastYearMTD, 
CASE WHEN F.ItemSeqID > 54 THEN L.YTD * @raw ELSE L.YTD/@Less * @raw END AS LastYearYTD,
CASE WHEN F.ItemSeqID > 54 THEN F.MTD * @raw - MTDBudgetTotal ELSE (F.MTD - MTDBudgetTotal)/@Less * @raw END AS MTDVariance,
CASE WHEN F.ItemSeqID > 54 THEN CASE WHEN YTDBudgetTotal IS NULL THEN F.YTD * @raw ELSE (F.YTD - YTDBudgetTotal) * @raw end
	ELSE (CASE WHEN YTDBudgetTotal IS NULL THEN F.YTD ELSE F.YTD - YTDBudgetTotal end)/@Less * @raw END AS YTDVariance,
CASE WHEN F.ItemSeqID > 54 THEN CASE WHEN L.YTD IS NULL THEN F.YTD ELSE (F.YTD - L.YTD) * @raw end 
	ELSE (CASE WHEN L.YTD IS NULL THEN F.YTD ELSE F.YTD - L.YTD end)/@Less * @raw END AS LY_YTDVariance
FROM 
