ALTER PROCEDURE [dbo].[procReportFinanceSAPPNLByMonth]
@CoyID smallint, 
@ProjectID smallint,
...
from tbFinanceDataSAP f WITH (NOLOCK) left outer join tbFinanceItem i WITH (NOLOCK) on f.ItemID = i.ItemID 
where f.CoyID = @CoyID and f.ProjectID = @ProjectID and f.DepartmentID = @DepartmentID and f.SectionID = @SectionID and f.UnitID = @UnitID
...
ItemSeqID, ItemSN, REPLACE(ItemName,'(PNL)','') as ItemName,
	...
	CASE WHEN ItemName = 'Gross Profits Margin %' THEN @GPM 
		 ...
		 WHEN ItemName = 'A&G Margin Direct %' THEN @AGMD
	...
	CASE WHEN ItemName = 'Gross Profits Margin %' THEN @LY_GPM 
		 ...
		 WHEN ItemName = 'A&G Margin Direct %' THEN @LY_AGMD
