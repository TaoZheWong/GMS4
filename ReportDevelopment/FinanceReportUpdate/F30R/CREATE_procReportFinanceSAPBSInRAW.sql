ALTER PROCEDURE [dbo].[procReportFinanceSAPBSInRAW]
@CoyID smallint, 
@Year smallint,
@Month smallint
AS 

DECLARE @CurrencyCode nvarchar(15),@CurrencySign nvarchar(5), @Company nvarchar(100), @Less Int, @raw smallint,
...
CASE WHEN F.ItemSeqID > 61 THEN F.MTD ELSE F.MTD/@Less * @raw END AS MTD, 
CASE WHEN F.ItemSeqID > 61 THEN LM_MTD ELSE LM_MTD/@Less * @raw END AS LM_MTD, 
CASE WHEN F.ItemSeqID > 61 THEN LLM_MTD ELSE LLM_MTD/@Less * @raw END AS LLM_MTD, 
CASE WHEN F.ItemSeqID > 61 THEN LY_MTD ELSE LY_MTD/@Less * @raw END AS LY_MTD, 
CASE WHEN F.ItemSeqID > 61 THEN LLY_MTD ELSE LLY_MTD/@Less END AS LLY_MTD, 
CASE WHEN @CurrencyCode = 'SGD' OR @Conversion IS NULL THEN 0 ELSE F.MTD / @Conversion * @raw END AS MTD_SGD
FROM 
...
LEFT OUTER JOIN tbFinanceDataSAP f ON p.ItemID = f.ItemID 
AND f.CoyID = @CoyID
AND f.ProjectID = -1 
AND f.DepartmentID = -1
AND f.SectionID = -1
