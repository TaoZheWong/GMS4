ALTER PROCEDURE [dbo].[procReportFinancePNLInSGD]
@CoyID smallint, 
@ProjectID smallint,
...
BEGIN
	EXEC procReportFinanceSAPPNLInSGD @CoyID, @ProjectID, @DepartmentID, @SectionID, @Year, @Month, @UnitID
	RETURN 
END
...
SELECT @Company as Company, UPPER(@Division) as Division, UPPER(@Department) as Department, UPPER(@Section) as Section, UPPER(@Unit) as Unit,
